...
class Brand(Base):
    ...
    examples = relationship(
        "BrandExample", back_populates="brand", cascade="all, delete-orphan"
    )
    content_plans = relationship(
        "ContentPlan", back_populates="brand", cascade="all, delete-orphan"
    )
...
class ContentPlan(Base):
    ...
    mode = Column(
        String(20), default="campaign"
    )  # campaign (المعمارية الجديدة) | legacy
    ...
    posts = relationship(
        "GeneratedPost", back_populates="content_plan", cascade="all, delete-orphan"
    )
...
class GeneratedPost(Base):
    ...
    post_id = Column(
        String(50)
    )  # المعرّف الأساسي الثابت للبوست (post_001...) عبر كل المراحل
    idea = Column(
        JSON, default=dict
    )  # الفكرة القانونية المشتركة (مصدر الحقيقة الوحيد للبوست)
...
class PostPerformanceSnapshot(Base):
    ...
    generated_post_id = Column(
        Integer, ForeignKey("generated_posts.id"), nullable=False
    )
    ...
    generated_post = relationship(
        "GeneratedPost", back_populates="performance_snapshots"
    )
...
class ScheduledPost(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    generated_post_id = Column(
        Integer, ForeignKey("generated_posts.id"), nullable=True
    )  # ربط بمنشور الحملة (للجدولة التلقائية)
    hook = Column(Text)
    caption = Column(Text)
    cta = Column(Text)
    hashtags = Column(JSON, default=list)
